"""
viz_3d_interactive.py  --  EDA/zelev  (Z-elevation targets)
Interactive 3-D Plotly visualization of the predicted SS layer.

Output: outputs/viz_3d_interactive.html  (opens in default browser)
"""
...
fig.add_trace(go.Surface(
    ...
    hovertemplate=(
        "X: %{x:.0f}<br>Y: %{y:.0f}<br>"
        "FW elev: %{z:.0f} m asl<extra>FW Contact</extra>"
    ),
))
...
fig.add_trace(go.Scatter3d(
    ...
    name="True HW",
    hovertemplate=(
        "Hole: %{text}<br>HW elev: %{z:.0f} m asl<extra>True HW</extra>"
    ),
    text=trg["Drillhole"].values,
))
...
fig.add_trace(go.Scatter3d(
    ...
    name="True FW",
    hovertemplate=(
        "Hole: %{text}<br>FW elev: %{z:.0f} m asl<extra>True FW</extra>"
    ),
    text=trg["Drillhole"].values,
))
...
fig.update_layout(
    title=dict(
        text="GNN Predicted SS Layer — Z Elevation Contacts (m asl)",
